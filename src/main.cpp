...
double feasibility_cost(int lane){
	double cost = 0;
	if (lane < 0 || lane > 2){
		return feasibility;
	}
		  ...
		  if (d<(2+4*lane+2) && d >(2+4*lane-2))
			{
				...
				double other_car_s = SensorVector[i][5];
...
double calculate(vector<vector<double>> SensorVector, int lane, int prev_size, double car_s){
	double cost = 0;
	...
	cout << "Left Lane Cost: " << cost_Map[0] << endl;
	cout << "Right Lane Cost: " << cost_Map[1] << endl;
	cout << "Keep Lane Cost: " << cost_Map[2] << endl;
	auto x = std::min_element(cost_Map.begin(), cost_Map.end(),[](const pair<int, int>& p1, const pair<int, int>& p2) {
            return p1.second < p2.second; });
  	if (cost_Map[0] >= danger && cost_Map[1] >= danger && cost_Map[2] >= danger){
    	return 3;
    }
    else if((cost_Map[0] == cost_Map[2])){
    	return 2;
    }
    return x->first;
}
...
int main() {
  ...
  //Prev step Speed/acceleration
  double prev_speed =0;
  double prev_acc =0;
  //Curr step Speed/time
  double curr_speed = 0;
  int lane = 1;
  ...
  h.onMessage([&start,&slowdown,&curr_speed, &lane,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&prev_speed,& prev_acc](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
          ...
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	for (int i=0;i<sensor_fusion.size();++i){
          		...
          		SensorVector.push_back(TempVec);
